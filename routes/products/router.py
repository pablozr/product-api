...
@router.get("/products")
async def get_products_endpoint(category: Optional[str] = None, sortby: Optional[str] = None, skip: int = 0, limit: int = 10):
    filters = {
        "category": category,
        "sortby": sortby
    }
    ...
    try:
        response_get_products = await db_instance.get_products(skip=skip, limit=limit, filters=filters)
